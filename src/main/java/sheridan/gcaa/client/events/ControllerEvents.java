    ...
    public static void onMouseScroll(InputEvent.MouseScrollingEvent event) {
        if (shouldHandleInputEvent() && Clients.isInAds() && Clients.MAIN_HAND_STATUS.getEffectiveSight() instanceof Scope scope) {
            ...
            if (!Float.isNaN(scopeMagnification)) {
                boolean zoomIn = event.getScrollDeltaY() < 0;
            ...
            if (GCAA.ALLOW_DEBUG && KeyBinds.OPEN_DEBUG_SCREEN.isDown() && event.getAction() == 1) {
                ...
                if (stackMain.getItem() instanceof IGun gun && !ReloadingHandler.isReloading()) {
                    if (gun.getGunProperties().fireModes.size() > 1) {
                        ...
                        PacketDistributor.sendToServer(new SwitchFireModePacket());
                        // Update client-side immediately for instant visual feedback
                        gun.switchFireMode(stackMain);
                        // Show current fire mode on screen
                        IGunFireMode currentMode = gun.getFireMode(stackMain);
                        Minecraft.getInstance().gui.setOverlayMessage(
                            Component.translatable("tooltip.fire_mode." + currentMode.getName().toLowerCase()),
                            false
                        );
                    }
                }
                ...
                if (!ReloadingHandler.isReloading() && !HandActionHandler.INSTANCE.hasTask() && stack.getItem() instanceof IGun gun) {
                    ...
                    PacketDistributor.sendToServer(new TurnFlashlightPacket(!on));
                ...
                PacketDistributor.sendToServer(new OpenGunModifyScreenPacket());
