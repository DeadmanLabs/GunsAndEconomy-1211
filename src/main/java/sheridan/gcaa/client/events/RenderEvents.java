...
public class RenderEvents {
    private static final ResourceLocation CHAMBER_EMPTY = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "textures/gui/screen_layout_icon/chamber_empty.png");
    private static final ResourceLocation CHAMBER_FILLED = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "textures/gui/screen_layout_icon/chamber_filled.png");
    private static final ResourceLocation HAS_GRENADE = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "textures/gui/screen_layout_icon/has_grenade.png");
                ...
                if (flashlight == null) {
                    try {
                        PostChain postChain = new PostChain(Minecraft.getInstance().getTextureManager(),
                                ...
                                ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "shaders/post/flashlight.json"));
                    ...
                    flashlight.process((float) event.getPartialTick().getGameTimeDeltaPartialTick(false));
    ...
    public static void onRenderHandFP(RenderHandEvent event) {
        if (event.getHand() == InteractionHand.MAIN_HAND) {
            ...
            if (Clients.MAIN_HAND_STATUS.holdingGun.get()) {
                GlobalWeaponBobbing.INSTANCE.update((float) event.getPartialTick(), status.equipProgress);
            }
        }
    }
            ...
            if (Clients.holdingGun()) {
                ...
                if (stack.getItem() instanceof IGun gun) {
                    if (!Clients.MAIN_HAND_STATUS.ads && !Clients.shouldHideFPRender && !SprintingHandler.INSTANCE.isSprinting()) {
                        CrossHairRenderer.INSTANCE.render(16, gun, event.getGuiGraphics(), player, stack, (float) event.getPartialTick().getGameTimeDeltaPartialTick(false),
    ...
    private static final ByteBufferBuilder GUN_MODEL_BUFFER = new ByteBufferBuilder(1024);
    ...
    public static void onFovCompute(ViewportEvent.ComputeFov event) {
        float adsProgress = Clients.MAIN_HAND_STATUS.getLerpAdsProgress((float) event.getPartialTick());
