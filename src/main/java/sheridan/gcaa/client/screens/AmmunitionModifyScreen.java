...
public class AmmunitionModifyScreen extends AbstractContainerScreen<AmmunitionModifyMenu> {
    private static final ResourceLocation BACKGROUND = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "textures/gui/screen/ammunition_modify.png");
    private static final ResourceLocation AMMO_PROGRESS_EMPTY = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "textures/gui/component/ammo_progress_empty.png");
    private static final ResourceLocation AMMO_PROGRESS_FILLED = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "textures/gui/component/ammo_progress_filled.png");
    private static final ResourceLocation MODIFY_AMMUNITION = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "textures/gui/component/modify_ammunition.png");
    private static final ResourceLocation BTN_BORDER = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "textures/gui/component/btn_border.png");
    ...
    private void applyModify() {
        if (checkAmmo() && selectedMods != null && selectedMods.size() > 0) {
            ...
            for (AmmunitionModRegister.ModEntry entry : selectedMods) {
                modNames.add(entry.mod().getId().toString());
            }
            PacketDistributor.sendToServer(new ApplyAmmunitionModifyPacket(modNames, price));
            needUpdate = true;
        }
    }
    ...
    public void updateClient(String modsUUID, int maxModCapability, CompoundTag modsTag, long balance) {
        ...
        if (checkAmmo()) {
            ...
            CompoundTag tag = item.getOrDefault(ModDataComponents.AMMO_DATA, new CompoundTag());
            tag.putString("modsUUID", modsUUID);
            tag.putInt("max_mod_capacity", maxModCapability);
            tag.put("mods", modsTag);
            item.set(ModDataComponents.AMMO_DATA, tag);
        }
        if (this.minecraft != null && this.minecraft.player != null) {
    ...
    private class ModIcon extends ImageButton {
        ...
        public ModIcon(int pX, int pY, int pWidth, int pHeight, OnPress pOnPress, AmmunitionModRegister.ModEntry entry)  {
            super(pX, pY, pWidth, pHeight, new WidgetSprites(
                    ResourceLocation.fromNamespaceAndPath("minecraft", "widget/button"),
                    ResourceLocation.fromNamespaceAndPath("minecraft", "widget/button_highlighted")
            ), pOnPress);
