...
public class ClientAttachmentsStatus {
    ...
    public void onSwitchEffectiveSight() {
        if (sights.size() > 1) {
            ...
            tempSightAimPos = null;
            PacketDistributor.sendToServer(new SetEffectiveSightPacket(effectiveSight.getId()));
        } else if (sights.size() == 1) {
            String sightUUID = gun.getEffectiveSightID(itemStack);
            if (!sightUUID.equals(sights.get(0).getId())) {
                PacketDistributor.sendToServer(new SetEffectiveSightPacket(sights.get(0).getId()));
            }
        }
    }
    ...
    public void sendSetScopeMagnificationPacket() {
        if (!Float.isNaN(scopeMagnificationRate) && originalScopeMagnification != scopeMagnificationRate && effectiveSight != null) {
            PacketDistributor.sendToServer(new SetScopeMagnificationPacket(effectiveSight.getAttachmentId(), scopeMagnificationRate));
