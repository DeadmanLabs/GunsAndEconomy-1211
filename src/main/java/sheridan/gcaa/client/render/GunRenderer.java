...
public class GunRenderer{
    ...
    public static void renderWithEntity(LivingEntity entityIn, PoseStack stackIn, ItemStack itemStackIn, ItemDisplayContext type, MultiBufferSource bufferIn, IGun gun, int combinedLightIn, int combinedOverlayIn, boolean leftHand, IGunModel model, DisplayData displayData) {
        ...
        if (model != null && displayData != null) {
            ...
            if (isFirstPerson) {
                // Extract view rotation from stackIn but use clean coordinate space
                // This preserves camera rotation while avoiding vanilla item scale/translation
                PoseStack poseStack = new PoseStack();
                org.joml.Quaternionf viewRotation = stackIn.last().pose().getNormalizedRotation(new org.joml.Quaternionf());
                ...
                if (dis < 35) {
                    float particleTick = Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(false);
