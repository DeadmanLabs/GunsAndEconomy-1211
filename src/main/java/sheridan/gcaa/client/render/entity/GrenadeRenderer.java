...
public class GrenadeRenderer extends EntityRenderer<Grenade> {
    ...
    private static final GrenadeModel<?> MODEL = new GrenadeModel<>(ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "model_assets/entities/grenade/grenade.geo.json"));
    ...
    public void render(Grenade pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        MODEL.renderToBuffer(pPoseStack, pBuffer.getBuffer(RenderType.entityCutout(this.getTextureLocation(pEntity))), pPackedLight, 655360, 0xFFFFFFFF);
        pPoseStack.popPose();
    }
