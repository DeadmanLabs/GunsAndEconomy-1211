...
public class BufferedModelBone implements IAnimatedModelPart{
    ...
    public com.mojang.blaze3d.vertex.MeshData drawState;
    ...
    private void compileVertexBuffer(PoseStack.Pose pose) {
        ...
        ByteBufferBuilder byteBuffer = new ByteBufferBuilder(256 * 256);
        buffer = new BufferBuilder(byteBuffer, renderType.mode(), renderType.format());
        VertexConsumer vertexConsumer = buffer;
        meshData.compile(pose, vertexConsumer);
        drawState = buffer.buildOrThrow();
        if (drawState != null) {
            vertexBuffer.bind();
            vertexBuffer.upload(drawState);
            VertexBuffer.unbind();
        }
    }
    ...
    public void render(int light) {
        ...
        RenderSystem.getModelViewStack().translate(this.x, this.y, this.z);
        if (this.xRot != 0.0F || this.yRot != 0.0F || this.zRot != 0.0F) {
            RenderSystem.getModelViewStack().rotate((new Quaternionf()).rotationZYX(this.zRot, this.yRot, this.xRot));
        }
        if (this.xScale != 1.0F || this.yScale != 1.0F || this.zScale != 1.0F) {
            RenderSystem.getModelViewStack().scale(this.xScale, this.yScale, this.zScale);
        }
        ...
        if (vertexBuffer != null && shader != null) {
            ...
            if (Clients.IS_IRIS_SHADER_ENABLED) {
                ...
                applyLightModify(light, tempStack);
                vertexBuffer.drawWithShader(RenderSystem.getModelViewMatrix(), RenderSystem.getProjectionMatrix(), shader);
                VertexBuffer.unbind();
    ...
    public static void __test_render__(int light) {
        if (TEST != null) {
            if (lastShaderEnabled != Clients.IS_SHADER_ENABLED) {
                TEST.compile(RenderTypes.getMeshCutOut(ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "model_assets/test_gltf/m1a2.png")));
            }
...
//            }
    ...
    public static class BoneMeshData {
