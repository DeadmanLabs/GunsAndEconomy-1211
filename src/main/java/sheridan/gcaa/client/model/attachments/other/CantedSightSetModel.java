...
public class CantedSightSetModel implements IAttachmentModel, ISlotProviderModel {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "model_assets/attachments/others/canted_sight_set.png");
    private final ModelPart root, s_canted_scope;

    public CantedSightSetModel() {
        root = ArsenalLib.loadBedRockGunModel(
                ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "model_assets/attachments/others/canted_sight_set.geo.json")).bakeRoot().getChild("root");
        root.meshingAll();
