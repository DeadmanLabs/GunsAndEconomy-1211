...
public class GunPropertiesHandler extends SimplePreparableReloadListener<Map<String, JsonObject>> {
    ...
    protected @NotNull Map<String, JsonObject> prepare(@NotNull ResourceManager pResourceManager, @NotNull ProfilerFiller pProfiler) {
        ...
        for (Gun gun : Gun.getAllInstances()) {
            ...
            String forgeKey = String.valueOf(BuiltInRegistries.ITEM.getKey(gun));
            String name = "gun_properties/" + fileName + ".json";
            ResourceLocation location = ResourceLocation.fromNamespaceAndPath(gun.id, name);
    ...
    public static void onDataPackSync(OnDatapackSyncEvent event) {
        if (!loadedProperties.isEmpty()) {
            ...
            if (player == null) {
                PacketDistributor.sendToAllPlayers(new UpdateGunPropertiesPacket(cache, strCache));
            } else {
                PacketDistributor.sendToPlayer(player, new UpdateGunPropertiesPacket(cache, strCache));
            }
        }
    }
