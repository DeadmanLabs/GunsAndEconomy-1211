...
public class GunPropertiesProvider implements DataProvider {
    ...
    public @NotNull CompletableFuture<?> run(@NotNull CachedOutput pOutput) {
        ...
        for (IGun gun : Gun.getAllInstances()) {
            ...
            ResourceLocation key = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, name);
            Path path = this.pathProvider.json(key);
            list.add(DataProvider.saveStable(pOutput, object, path));
        }
        JsonObject object = new JsonObject();
        genProtocol(object);
        list.add(DataProvider.saveStable(pOutput, object, pathProvider.file(ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "readme"), "txt")));
