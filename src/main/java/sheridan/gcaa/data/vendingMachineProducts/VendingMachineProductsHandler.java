...
public class VendingMachineProductsHandler extends SimplePreparableReloadListener<Map<String, List<JsonObject>>> {
    ...
    protected @NotNull Map<String,  List<JsonObject>> prepare(@NotNull ResourceManager pResourceManager, @NotNull ProfilerFiller pProfiler) {
        ...
        ResourceLocation location = ResourceLocation.fromNamespaceAndPath(GCAA.MODID, "vending_machine_products/vending_machine_products_register.json");
    ...
    public static void onDataPackSync(OnDatapackSyncEvent event) {
        ...
        if (player == null) {
            PacketDistributor.sendToAllPlayers(new UpdateVendingMachineProductsPacket(strCache, cache));
        } else {
            PacketDistributor.sendToPlayer(player, new UpdateVendingMachineProductsPacket(strCache, cache));
        }
    }
    ...
    private void genCache() {
        ...
        try {
            ...
            strCache = ""; // Use empty string instead of null for packet encoding
        } catch (Exception e) {
            cache = new byte[0]; // Use empty array instead of null for packet encoding
            strCache = string;
        }
    }
