...
public class GrenadeLauncher extends Attachment implements IArmReplace, IInteractive, ForwardSlotBlocker {
    ...
    public GrenadeLauncher(IAmmunition ammunition, InertialRecoilData recoilData, float recoilPitch, float recoilYaw,
                           DeferredHolder<SoundEvent, SoundEvent> fireSound, int reloadTicks, float velocity, float pInaccuracy,
    ...
    protected void handleClientShoot(ItemStack stack, IGun gun, Player player, GrenadeLauncher launcher) {
        ...
        PacketDistributor.sendToServer(new FireGrenadeLauncherPacket(now, Item.getId(launcher)));
        setHasGrenade(stack, gun, false);
    }
    ...
    @Override
    public void appendHoverText(@NotNull ItemStack pStack, @NotNull Item.TooltipContext pLevel, @NotNull List<Component> pTooltipComponents, @NotNull TooltipFlag pIsAdvanced) {
