...
public class Ammunition extends NoRepairNoEnchantmentItem implements IAmmunition{
    ...
    public Ammunition(int capacity, int modCapacity, Set<IAmmunitionMod> suitableMods)  {
        super(new Properties().durability(capacity).setNoRepair());
    ...
    public static void clientManageAmmunition(Player player, ItemStack stack) {
        if (System.currentTimeMillis() - lastAmmunitionManageTime > AMMUNITION_MANAGE_DELAY) {
            PacketDistributor.sendToServer(new AmmunitionManagePacket());
    ...
    public void addMod(IAmmunitionMod mod, ItemStack itemStack) {
        ...
        if (maxModCapacity - capacity - mod.getCostFor(this) >= 0) {
            ...
            itemStack.set(sheridan.gcaa.items.ModDataComponents.AMMO_DATA, tag);
    ...
    public CompoundTag checkAndGet(ItemStack itemStack) {
        CompoundTag nbt = itemStack.getOrDefault(sheridan.gcaa.items.ModDataComponents.AMMO_DATA, new CompoundTag());
        if (nbt.isEmpty() || !nbt.contains("mods") || !nbt.contains(DATA_RATE))  {
            this.onCraftedBy(itemStack, null, null);
            nbt = itemStack.getOrDefault(sheridan.gcaa.items.ModDataComponents.AMMO_DATA, new CompoundTag());
        }
        return nbt;
    }
    ...
    public void onCraftedBy(@NotNull ItemStack pStack, @NotNull Level pLevel, @NotNull Player pPlayer) {
        ...
        CompoundTag nbt = pStack.getOrDefault(sheridan.gcaa.items.ModDataComponents.AMMO_DATA, new CompoundTag());
        ...
        pStack.set(sheridan.gcaa.items.ModDataComponents.AMMO_DATA, nbt);
    }
